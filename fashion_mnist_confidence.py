...
LEARNING_RATE = 0.001
MAX_PERCENTAGE = 20
OUTPUT_DIR_RANDOM = Path("output_fashion/confidence/random")
OUTPUT_DIR_WISE = Path("output_fashion/confidence/wise")
...
def plot_tsne(features, labels, title, filename):
    ...
    scatter = plt.scatter(tsne_results[:, 0], tsne_results[:, 1], c=labels, cmap=plt.get_cmap("jet", num_classes))
...
def main():
    transform = transforms.Compose([
        transforms.ToTensor(),
        transforms.Normalize((0.5,), (0.5,)) # FashionMNIST stats are closer to 0.5
    ])
    ...
    test_indices, val_indices, unlabeled_pool_indices = indices[:test_split_idx], indices[test_split_idx:val_split_idx], indices[val_split_idx:]
    test_set, val_set, unlabeled_pool = Subset(full_train_dataset, test_indices), Subset(full_train_dataset, val_indices), Subset(full_train_dataset, unlabeled_pool_indices)
    test_loader, val_loader = DataLoader(test_set, batch_size=BATCH_SIZE, shuffle=False), DataLoader(val_set, batch_size=BATCH_SIZE, shuffle=False)
    print(f"Test set: {len(test_set)}, Val set: {len(val_set)}, Unlabeled pool: {len(unlabeled_pool)}")
    ...
    results = {"Percentage": [0], "Random Test F1": [baseline_test_f1], "Wise Test F1": [baseline_test_f1], "Random Val F1": [baseline_val_f1], "Wise Val F1": [baseline_val_f1]}
    
    baseline_model.eval()
    test_features, test_labels = [], []
    with torch.no_grad():
        for data, target in test_loader:
            features = baseline_model.get_features(data.to(DEVICE))
            test_features.append(features.cpu().numpy())
            test_labels.append(target.cpu().numpy())
    test_features = np.concatenate(test_features, axis=0)
    test_labels = np.concatenate(test_labels, axis=0)
    tsne_title = f"FashionMNIST Baseline (Untrained) – Test F1={baseline_test_f1:.4f}"
    ...
    acquired_indices_random, acquired_indices_wise = [], []
    remaining_pool_indices_random, remaining_pool_indices_wise = list(unlabeled_pool_indices), list(unlabeled_pool_indices)

    criterion = nn.CrossEntropyLoss()
    for p in tqdm(range(1, MAX_PERCENTAGE + 1), desc="Active Learning Iterations"):
        ...
        tsne_title_wise = f"Wise (Confidence) – {p}% Trained – Test F1={test_f1_wise:.4f}"
        ...
        print("\n--- Current F1 Score Summary (Confidence) ---")
        print(df)

    print("\n--- Final F1 Score Summary (Confidence) ---")
    df = pd.DataFrame(results)
    df.to_csv(OUTPUT_DIR_WISE / "results_confidence.csv", index=False)
    print(df)
